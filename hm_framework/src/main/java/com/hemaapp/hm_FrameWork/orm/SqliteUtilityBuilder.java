...
public class SqliteUtilityBuilder {
    ...
    public SqliteUtility build(Context context) {
        ...
        if (sdcardDb) {
            ...
            XtomLogger.d(TAG, "打开app数据库" + dbName + ", version = " + db.getVersion());
        } else {
            db = new SqliteDbHelper(context, dbName, version).getWritableDatabase();


            XtomLogger.d(TAG, "打开sdcard库" + dbName + ",version=" + db.getVersion());
        }
    ...
    static SQLiteDatabase openSdcardDb(String path, String dbName, int version) {
        ...
        if (dbf.exists()) {
            ...
            try {
                if (dbf.createNewFile()) {
                    XtomLogger.d(TAG, "新建一个库再sd卡，库名 = " + dbName + ",路径 = " + dbf.getAbsolutePath());
                    db = SQLiteDatabase.openOrCreateDatabase(dbf, null);
                }
        ...
        if (db != null) {
            ...
            XtomLogger.d(TAG, "表" + dbName + "的version" + dbVersion + ", newVersion =" + version);
    ...
    static void dropDb(SQLiteDatabase db) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                XtomLogger.d(TAG, "删除表 = " + cursor.getString(0));
            }
        }
        if (cursor != null) {
            cursor.close();
