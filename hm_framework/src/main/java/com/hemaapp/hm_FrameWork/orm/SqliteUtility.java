...
public class SqliteUtility {
    ...
    SqliteUtility(String dbName, SQLiteDatabase db) {
        ...
        XtomLogger.d(TAG, "将库" + dbName + "放入到缓存中。");
    }
    ...
    public <T> List<T> select(Class<T> clazz, String selection,
        ...
        XtomLogger.d(TAG, " method[select], table[" + tableInfo.getTableName() + "], selection[" + selection + "], selectionArgs" + JSON.toJSON(selectionArgs) + ", groupBy[" + String.valueOf(groupBy) + "], having[" + String.valueOf(having) + "], orderBy[" + String.valueOf(orderBy) + "], limit[" + String.valueOf(limit) + "]");
        ...
        Cursor cursor;
        try {
            cursor = db.query(tableInfo.getTableName(), columnList.toArray(new String[0]),
                    selection, selectionArgs, groupBy, having, orderBy, limit);
        } catch (Exception e) {
            e.printStackTrace();
            return list;
        }
        XtomLogger.d(TAG, "table[" + tableInfo.getTableName() + "] 查询数据结束，耗时 " + String.valueOf(System.currentTimeMillis() - start) + " ms");
        ...
        XtomLogger.d(TAG, "table[" + tableInfo.getTableName() + "], 设置数据结束，耗时 " + String.valueOf(System.currentTimeMillis() - start) + " ms");

        XtomLogger.d(TAG, "查询到数据 " + list.size() + " 条");
    ...
    public <T> void insert(Extra extra, T... entities) {
        try {
            if (entities != null && entities.length > 0)
                ...
                XtomLogger.d(TAG, "method[insert(Extra extra, T... entities)], entities is null or empty");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public <T> void insertOrReplace(Extra extra, T... entities) {
        try {
            if (entities != null && entities.length > 0)
                ...
                XtomLogger.d(TAG, "method[insertOrReplace(Extra extra, T... entities)], entities is null or empty");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private <T> void insert(Extra extra, List<T> entityList, String insertInto) {
        if (entityList == null || entityList.size() == 0) {
            XtomLogger.d(TAG, "method[insert(Extra extra, List<T> entityList)], entityList is null or empty");
            return;
        }
        ...
        try {
            ...
            XtomLogger.v(TAG, insertInto + " sql = " + sql);
            ...
            XtomLogger.d(TAG, "bindvalues 耗时 " + bindTime + " ms");
        ...
        XtomLogger.d(TAG, "表 " + tableInfo.getTableName() + insertInto + " 数据 " + entityList.size() + " 条， 执行时间 " + String.valueOf(System.currentTimeMillis() - start) + " ms");
    }
    ...
    public <T> void update(Extra extra, T... entities) {
        try {
            if (entities != null && entities.length > 0)
                ...
                XtomLogger.d(TAG, "method[update(Extra extra, T... entities)], entities is null or empty");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public <T> void deleteAll(Extra extra, Class<T> clazz) {
        try {
            ...
            XtomLogger.d(TAG, "method[delete] table[" + tableInfo.getTableName() + "], sql[" + sql + "]");

            long start = System.currentTimeMillis();
            db.execSQL(sql);
            XtomLogger.d(TAG, "表 " + tableInfo.getTableName() + " 清空数据, 耗时 " + String.valueOf(System.currentTimeMillis() - start) + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public <T> void deleteById(Extra extra, Class<T> clazz, Object id) {
        try {
            ...
            XtomLogger.d(TAG, " method[deleteById], table[" + tableInfo.getTableName() + "], id[" + String.valueOf(id) + "], whereClause[" + whereClause + "], whereArgs" + JSON.toJSON(whereArgs) + " ");

            long start = System.currentTimeMillis();
            int rowCount = db.delete(tableInfo.getTableName(), whereClause, whereArgs);

            XtomLogger.d(TAG, "表 " + tableInfo.getTableName() + " 删除数据 " + rowCount + " 条, 耗时 " + String.valueOf(System.currentTimeMillis() - start) + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public <T> void delete(Class<T> clazz, String whereClause, String[] whereArgs) {
        try {
            ...
            XtomLogger.d(TAG, "method[delete], table[" + tableInfo.getTableName() + "], whereClause[" + whereClause + "], whereArgs" + JSON.toJSON(whereArgs) + " ");
            XtomLogger.d(TAG, "表 " + tableInfo.getTableName() + " 删除数据 " + rowCount + " 条，耗时 " + String.valueOf(System.currentTimeMillis() - start) + " ms");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public long sum(Class<?> clazz, String column, String whereClause, String[] whereArgs) {
        ...
        XtomLogger.d(TAG, "sum() --- > " + sql);
        XtomLogger.d(TAG, "whereArgs:" + whereArgs);
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                XtomLogger.d(TAG, "sum = " + String.valueOf(sum) + " 耗时" + String.valueOf(System.currentTimeMillis() - time) + "ms");
                cursor.close();
                return sum;
            }
        } catch (Exception e) {
            XtomLogger.v(TAG, e.getMessage());
        }
        return 0;
    }
    ...
    public long count(Class<?> clazz, String whereClause, String[] whereArgs) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                XtomLogger.d(TAG, "count = " + String.valueOf(count) + " 耗时" + String.valueOf(System.currentTimeMillis() - time) + "ms");
                cursor.close();
                return count;
            }
        } catch (Exception e) {
    ...
    private <T> void bindInsertValue(SQLiteStatement insertStatement, int index, TableColumn column, T entity) {
        ...
        try {
            ...
            XtomLogger.w(TAG, "属性 " + column.getField().getName() + " bindvalue 异常");
        }
    }
